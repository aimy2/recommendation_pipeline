...
def compute_and_upsert_neighbors(model, item_uniques):
    ...
    for item_idx in range(n_items):
        # map item id gracefully (don't force int())
        ...
        if isinstance(sims, tuple) and len(sims) == 2:
            try:
                ...
                # fallback: attempt conservative coercion
                try:
                    indices, scores = sims
        ...
        # Case C: list-like of pairs (the expected default)
            ...
            try:
                ...
                # last-resort attempt to iterate and coerce
                try:
                    for el in sims:
        ...
        for sim_idx, score in pairs:
            ...
            if score <= MIN_SCORE:
                continue
            # map sim_idx to neighbor id (using item_uniques) if possible
            try:
                neighbor_raw = item_uniques[sim_idx]
                try:
                    neighbor_id = int(neighbor_raw)
                except Exception:
                    neighbor_id = neighbor_raw
            except Exception:
                # if mapping fails, try to use sim_idx directly
                try:
